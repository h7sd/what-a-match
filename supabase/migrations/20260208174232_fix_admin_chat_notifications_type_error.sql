/*
  # Fix Admin Chat Notifications Type Error

  This migration fixes the type mismatch in get_admin_chat_notifications function.

  1. Changes
    - Fix type comparison between text and uuid in the function
    - Cast assigned_admin_id to uuid for proper comparison
...
BEGIN
  ...
  AND (
    c.assigned_admin_id IS NULL 
    OR (c.assigned_admin_id IS NOT NULL AND c.assigned_admin_id::uuid = auth.uid())
  )
  ORDER BY 
