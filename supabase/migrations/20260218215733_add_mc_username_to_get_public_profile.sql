/*
  # Add mc_username to get_public_profile function

  The mc_username column was added to the profiles table but was never
  included in the get_public_profile (and get_public_profile_by_alias)
  function definitions, so it was never returned when loading a user profile.

  This migration recreates both functions to include mc_username.
*/
...
RETURNS TABLE(
  ...
  use_global_badge_color boolean,
...
AS $$
  SELECT
    ...
    p.use_global_badge_color,
    p.global_badge_color,
    p.username,
    p.views_count
  FROM profiles p
