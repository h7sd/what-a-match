...
serve(async (req) => {
  ...
  try {
    ...
    if (checkSuspiciousIp(clientIp).blocked) {
      return new Response(
        JSON.stringify({ error: "Your access has been temporarily blocked due to suspicious activity." }),
    ...
    const sanitizedMessages = (rawMessages as Array<{ role: string; content: string }>).map((msg) => ({
      role: msg.role,
    ...
    if (lastUserMessage) {
      ...
      if (check.detected) {
        ...
        return new Response(
          JSON.stringify({ error: "Diese Anfrage kann ich nicht bearbeiten. Bitte halte dich an die Nutzungsrichtlinien." }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    }
    ...
    if (!ipCheck.allowed) {
      return new Response(
        ...
        { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json", "Retry-After": "3600", "X-RateLimit-Remaining": "0" } }
      );
    }
